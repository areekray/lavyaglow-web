...
export function ProductForm({ product, onClose, onSave }: ProductFormProps) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, control, watch, setValue } = useForm<ProductFormData>({
    defaultValues: {
      name: product?.name || '',
  ...
  const { fields: priceSetFields, append: appendPriceSet, remove: removePriceSet } = useFieldArray({
    control,
  ...
  const watchActualPrice = !actualPrice ? 0 : typeof actualPrice === 'string' ? parseInt(actualPrice) : actualPrice;
  const watchDiscountedPrice = !discountedPrice ? 0 : typeof discountedPrice === 'string' ? parseInt(discountedPrice) : discountedPrice;
  ...
  const discountPercentage = productService.calculateDiscountPercentage(watchActualPrice, watchDiscountedPrice);
  ...
  if (watchActualPrice > 0 && priceSetFields.length > 0) {
    priceSetFields.forEach((_field, index) => {
      ...
      if (setQuantity) {
        ...
        const currentDiscounted = watch(`price_sets.${index}.discounted_price` as const);
        if (!currentDiscounted || currentDiscounted >= newActualPrice) {
          ...
          setValue(`price_sets.${index}.discounted_price` as const, suggestedDiscount);
        }
      }
    });
  }
...
// Update the addPriceSet function:
  ...
  const onSubmit = async (data: ProductFormData) => {
    try {
      // Validate pricing
      if (intParseHandler(data.actual_price) < intParseHandler(data.discounted_price)) {
        toast.error('Discounted price cannot be higher than actual price');
        return;
      }

      if (!watchCanDoBulk && data.in_stock && intParseHandler(data.stock_quantity) <= 0) {
        toast.error('Stock quantity must be greater than 0 when product is in stock and not bulk-enabled');
        return;
      }
      ...
      if (data.highlight_in_home && !product?.highlight_in_home) {
        if (highlightCount >= 4) {
          toast.error('Only 4 products can be featured in Our Curated Collection. Please remove highlighting from another product first.');
          return;
        }
      }
  ...
  return (
    <div className="product-form-overlay">
      <div className="product-form">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="product-form__content">
          ...
          <div className="product-form__section">
            ...
            <div className="form-group">
              ...
              />
              {errors.description && (
                <span className="form-group__error">{errors.description.message}</span>
              )}
            </div>
            ...
            <div className="product-form__row">
              ...
              <Input
                ...
                {...register('discounted_price', { 
                  ...
                  min: { value: 0.01, message: 'Price must be greater than 0' },
                  validate: value => value <= watchActualPrice || 'Discounted price cannot be higher than actual price'
                })}
              />
            </div>
            ...
            <div className="pricing-preview">
              ...
              <div className="pricing-preview__display">
                {discountPercentage > 0 ? (
                  <>
                    <span className="pricing-preview__actual">₹{watchActualPrice?.toFixed(2)}</span>
                    <span className="pricing-preview__discounted">₹{watchDiscountedPrice?.toFixed(2)}</span>
                    <span className="pricing-preview__discount">({discountPercentage}% off)</span>
                  </>
                ) : (
                  <span className="pricing-preview__single">₹{watchDiscountedPrice?.toFixed(2)}</span>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="stock-management">
                  ...
                  render={({ field }) => (
                    <label className="toggle-label">
                      ...
                      <span className="toggle-text">
                        Enable Bulk Orders
                        <small>When enabled, stock quantity is ignored and only availability matters</small>
                      </span>
                    </label>
                  )}
                />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <label className="toggle-label">
                      ...
                      <span className="toggle-text">
                        Product Available
                        {watchCanDoBulk && <small>Auto-enabled for bulk orders</small>}
                      </span>
                    </label>
                  )}
                />
              ...
              {!watchCanDoBulk && (
                <Input
                  ...
                  {...register('stock_quantity', { 
                    required: !watchCanDoBulk ? 'Stock quantity is required' : false,
              ...
              <div className="stock-status-info">
                ...
                <div className={`status-preview ${
                  !watchInStock ? 'status-preview--out-of-stock' :
                  watchCanDoBulk ? 'status-preview--bulk' : 'status-preview--in-stock'
                }`}>
                  {!watchInStock ? (
          ...
          <div className="product-form__section">
            <h3 className="product-form__section-title">Set Pricing (Bulk Discounts)</h3>
            <p className="product-form__section-description">
              Create pricing for different quantities. Customers get additional discounts when buying in sets.
            </p>
                ...
                <div key={field.id} className="price-set">
                  ...
                  <div className="price-set__row">
                    <Input
                      ...
                      type="number"
                      min="2"
                      {...register(`price_sets.${index}.set_quantity` as const, {
                        required: 'Quantity is required',
                        min: { value: 2, message: 'Minimum set quantity is 2' }
                      })}
                    ...
                    <Input
                      ...
                      type="number"
                      step="0.01"
                      {...register(`price_sets.${index}.actual_price` as const, {
                        required: 'Actual price is required',
                    ...
                    <Input
                      ...
                      type="number"
                      step="0.01"
                      {...register(`price_sets.${index}.discounted_price` as const, {
                        required: 'Discounted price is required',
                        min: { value: 0.01, message: 'Price must be positive' }
                      })}
                      error={errors.price_sets?.[index]?.discounted_price?.message}
                    />
                    <button
                      type="button"
                  ...
                  <div className="price-set__preview">
                    {watch(`price_sets.${index}.actual_price` as const) && watch(`price_sets.${index}.discounted_price` as const) && (
                      <span className="price-set__preview-text">
                        Set of {watch(`price_sets.${index}.set_quantity` as const)}: 
                        <span className="actual-price">₹{handleToFixed(watch(`price_sets.${index}.actual_price` as const))}</span>
                        <span className="discounted-price">₹{handleToFixed(watch(`price_sets.${index}.discounted_price` as const))}</span>
                        {productService.calculateDiscountPercentage(
                          watch(`price_sets.${index}.actual_price` as const) || 0, 
                          watch(`price_sets.${index}.discounted_price` as const) || 0
                        ) > 0 && (
          ...
          {/* Highlighting Section */}
            ...
            <Controller
                ...
                <label className="toggle-label">
                    ...
                    <small className={`highlight-message ${
                        watchHighlightInHome ? 'highlight-message--active' :
                        highlightCount >= 4 ? 'highlight-message--warning' : 'highlight-message--info'
                    }`}>
                        {highlightMessage}
                    </small>
                    </span>
